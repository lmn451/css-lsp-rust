...
| Eager JS | `--eager-js` | `CSS_LSP_EAGER_JS=1` | false | Parse unopened JS/TS CSS-in-JS files |
...
The `release.yml` workflow automatically builds binaries for all platforms:
...
```bash
cargo login  # Configure crates.io credentials
cargo publish
```
